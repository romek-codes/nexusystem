...
selected=$(rofi -i -dmenu -show-icons -matching fuzzy -sorting-method fzf -sort < <(
	...
	printf "%s\0icon\x1f%s\x1fmeta\x1f%s\n" \
		"Open browser" "${MAIN_BROWSER_ICON}" "zen firefox chrome web internet"
	printf "%s\0icon\x1f%s\x1fmeta\x1f%s\n" \
	...
	printf "%s\0icon\x1f%s\x1fmeta\x1f%s\n" \
		"Close tmux session" "utilities-terminal" "tmux session close kill"
	printf "%s\0icon\x1f%s\x1fmeta\x1f%s\n" \
